			...
			for _, container := range tt.containers {
				t.Run(container, func(t *testing.T) {
					...
					for _, sample := range samples {
						...
						// assert.Contains(t, sample, PostgresqlInstanceSample,
						// 	"Sample %d - Integration output does not contain PostgresqlInstanceSample", idx)
						...
						// t.Run(fmt.Sprintf("Validating JSON Schema For %s", tt.expectedSampleTypes[count]), func(t *testing.T) {
						// 	sampleType := tt.expectedSampleTypes[count]
						// 	if !strings.Contains(sample, sampleType) {
						// 		t.Errorf("Integration output does not contain: %s", tt.expectedSampleTypes[count])
						// 	}
