...
func Test_GetSlowRunningMetrics_Error(t *testing.T) {
	...
	metrics, metricsInterface, err := GetSlowRunningMetrics(conn, args, "someString", 123)
	assert.Error(t, err)
...
func Test_PopulateSlowRunningMetrics_NoExtension(t *testing.T) {
	...
	metrics := PopulateSlowRunningMetrics(conn, pgIntegration, args, "someString", 123)
	assert.Nil(t, metrics)
}
func Test_PopulateSlowRunningMetrics_NoSlowQueries(t *testing.T) {
	...
	slowQueryMetricsList := PopulateSlowRunningMetrics(conn, pgIntegration, args, "someString", 123)
...
// 	query := `SELECT 'newrelic' as newrelic, pss.queryid AS query_id, LEFT(pss.query, 4095) AS query_text, pd.datname AS database_name, current_schema() AS schema_name, pss.calls AS execution_count, ROUND((pss.total_exec_time / pss.calls)::numeric, 3) AS avg_elapsed_time_ms, pss.shared_blks_read / pss.calls AS avg_disk_reads, pss.shared_blks_written / pss.calls AS avg_disk_writes, CASE WHEN pss.query ILIKE 'SELECT%' THEN 'SELECT' WHEN pss.query ILIKE 'INSERT%' THEN 'INSERT' WHEN pss.query ILIKE 'UPDATE%' THEN 'UPDATE' WHEN pss.query ILIKE 'DELETE%' THEN 'DELETE' ELSE 'OTHER' END AS statement_type, to_char(NOW() AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"') AS collection_timestamp FROM pg_stat_statements pss JOIN pg_database pd ON pss.dbid = pd.oid WHERE pd.datname in (someString) AND pss.query NOT ILIKE 'EXPLAIN (FORMAT JSON) %' AND pss.query NOT ILIKE 'SELECT $1 as newrelic%' AND pss.query NOT ILIKE 'WITH wait_history AS%' AND pss.query NOT ILIKE 'select -- BLOATQUERY%' AND pss.query NOT ILIKE 'select -- INDEXQUERY%' AND pss.query NOT ILIKE 'SELECT -- TABLEQUERY%' AND pss.query NOT ILIKE 'SELECT table_schema%' ORDER BY avg_elapsed_time_ms DESC -- Order by the average elapsed time in descending order LIMIT 0;`
// 	mock.ExpectQuery(regexp.QuoteMeta(query)).WillReturnRows(sqlmock.NewRows([]string{"query_id", "query_text", "database_name", "schema_name", "execution_count", "avg_elapsed_time_ms", "avg_cpu_time_ms", "avg_disk_reads", "avg_disk_writes", "statement_type", "collection_timestamp"}).
// 		AddRow(1, "SELECT * FROM test", "testdb", "public", 10, 1500, 1500, 1, 1, "SELECT", "2023-10-10T10:10:10Z"))
