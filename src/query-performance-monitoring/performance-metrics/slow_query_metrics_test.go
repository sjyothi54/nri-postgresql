...
func TestPopulateSlowMetrics(t *testing.T) {
	...
	performancemetrics.PopulateSlowRunningMetrics(conn, pgIntegration, gv, nil)
...
func TestPopulateSlowMetricsInEligibility(t *testing.T) {
	...
	slowqueryList := performancemetrics.PopulateSlowRunningMetrics(conn, pgIntegration, gv, nil)
...
func runSlowQueryTest(t *testing.T, query string, version uint64, expectedLength int) {
	...
	slowQueryList, _, err := performancemetrics.GetSlowRunningMetrics(conn, gv, nil)
