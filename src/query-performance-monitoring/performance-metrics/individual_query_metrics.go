...
func GetIndividualQueryMetrics(conn *performancedbconnection.PGSQLConnection, slowRunningQueries []datamodels.SlowRunningQueryMetrics, cp *globalvariables.CommonParameters) ([]interface{}, []datamodels.IndividualQueryMetrics) {
	...
	for _, slowRunningMetric := range slowRunningQueries {
		...
		query := fmt.Sprintf(versionSpecificIndividualQuery, *slowRunningMetric.QueryID, cp.Databases, cp.Arguments.QueryMonitoringResponseTimeThreshold, cp.Arguments.QueryMonitoringCountThreshold)
