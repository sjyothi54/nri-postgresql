...
func PopulateWaitEventMetrics(conn *performancedbconnection.PGSQLConnection, pgIntegration *integration.Integration) error {
	isEligible, err := validations.CheckWaitEventMetricsFetchEligibility(conn, global_variables.Version)
	if err != nil {
	...
	commonutils.IngestMetric(waitEventMetricsList, "PostgresWaitEvents", pgIntegration)
	return nil
}
