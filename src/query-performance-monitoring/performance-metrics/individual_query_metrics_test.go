...
func Test_PopulateIndividualQueryMetrics_Success(t *testing.T) {
	...
	mock.ExpectQuery("SELECT count\\(\\*\\) FROM pg_extension WHERE extname = 'pg_stat_monitor'").WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(1))
	mock.ExpectQuery("SELECT query, queryid, datname, planid, ROUND\\(\\(\\(cpu_user_time \\+ cpu_sys_time\\) / NULLIF\\(calls, 0\\)\\)::numeric, 3\\) AS avg_cpu_time_ms FROM pg_stat_monitor WHERE queryid IN \\(1\\) GROUP BY query, queryid, datname, planid, cpu_user_time, cpu_sys_time, calls").WillReturnRows(sqlmock.NewRows([]string{"query", "queryid", "datname", "planid", "avg_cpu_time_ms"}).
