...
func TestPopulateIndividualQueryMetrics(t *testing.T) {
	...
	global_variables.IndividualQuery = queries.IndividualQuerySearchV13AndAbove
	...
	query := fmt.Sprintf(global_variables.IndividualQuery, mockQueryID, global_variables.DatabaseString, global_variables.Args.QueryResponseTimeThreshold, min(global_variables.Args.QueryCountThreshold, commonutils.MaxIndividualQueryThreshold))
	...
	individualQueryMetrics := performancemetrics.PopulateIndividualQueryMetrics(conn, slowRunningQueries, pgIntegration)
...
func TestGetIndividualQueryMetrics(t *testing.T) {
	...
	// Mock the individual query
	query := fmt.Sprintf(global_variables.IndividualQuery, mockQueryID, databaseName, args.QueryResponseTimeThreshold, min(args.QueryCountThreshold, commonutils.MaxIndividualQueryThreshold))
	mock.ExpectQuery(regexp.QuoteMeta(query)).WillReturnRows(sqlmock.NewRows([]string{
		"newrelic", "query", "queryid", "datname", "planid", "avg_cpu_time_ms", "avg_exec_time_ms",
	}).AddRow(
		"newrelic_value", "SELECT 1", "queryid1", "testdb", "planid1", 10.0, 20.0,
	...
	individualQueryMetricsInterface, individualQueryMetrics := performancemetrics.GetIndividualQueryMetrics(conn, slowRunningQueries)
