...
func Test_PopulateBlockingMetrics_IngestMetrics(t *testing.T) {
	...
	query := `SELECT
      ...
      WHERE NOT blocked_locks.granted
          ...
          AND blocking_statements.query NOT LIKE 'EXPLAIN (FORMAT JSON) %%'
      LIMIT 10;`
	mock.ExpectQuery(query).WillReturnRows(sqlmock.NewRows([]string{"blocked_pid", "blocked_query", "blocked_query_id", "blocked_query_start", "database_name", "blocking_pid", "blocking_query", "blocking_query_id", "blocking_query_start"}).
