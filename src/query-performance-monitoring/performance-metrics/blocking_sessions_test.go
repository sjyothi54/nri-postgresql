...
func TestPopulateBlockingMetrics(t *testing.T) {
	...
	err := performancemetrics.PopulateBlockingMetrics(conn, pgIntegration, gv, nil)
...
func TestGetBlockingMetrics(t *testing.T) {
	...
	blockingQueriesMetricsList, err := performancemetrics.GetBlockingMetrics(conn, gv, nil)
	assert.NoError(t, err)
