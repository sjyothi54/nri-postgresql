...
func Test_GetBlockingMetrics_Error(t *testing.T) {
	...
	metrics, err := GetBlockingMetrics(conn, args)
	assert.Error(t, err)
	assert.Nil(t, metrics)
}

func Test_GetBlockingMetrics_StructScanError(t *testing.T) {
	conn, mock := performanceDbConnection.CreateMockSQL(t)
	defer conn.Close()
	...
	mock.ExpectQuery(regexp.QuoteMeta(query)).WillReturnRows(sqlmock.NewRows([]string{"blocked_pid", "blocked_query", "blocked_query_id", "blocked_query_start", "database_name", "blocking_pid", "blocking_query", "blocking_query_id", "blocking_query_start"}).
		AddRow("invalid_pid", "SELECT * FROM test", 12345, "2023-10-10 10:10:10", "testdb", 2, "UPDATE test SET value = 1", 67890, "2023-10-10 10:10:20"))
...
func Test_PopulateBlockingMetrics_NoExtension(t *testing.T) {
	...
	PopulateBlockingMetrics(conn, pgIntegration, args)
	// No assertions needed as we are just testing the logging
}

func Test_PopulateBlockingMetrics_FetchError(t *testing.T) {
	conn, mock := performanceDbConnection.CreateMockSQL(t)
	defer conn.Close()
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT
      ...
      WHERE NOT blocked_locks.granted
          AND blocked_statements.query NOT LIKE 'EXPLAIN (FORMAT JSON) %%'
          AND blocking_statements.query NOT LIKE 'EXPLAIN (FORMAT JSON) %%'
      LIMIT 10;`)).WillReturnError(errors.New("query error"))
...
func Test_PopulateBlockingMetrics_NoBlockingQueries(t *testing.T) {
	conn, mock := performanceDbConnection.CreateMockSQL(t)
	defer conn.Close()
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT
      ...
      WHERE NOT blocked_locks.granted
          AND blocked_statements.query NOT LIKE 'EXPLAIN (FORMAT JSON) %%'
          AND blocking_statements.query NOT LIKE 'EXPLAIN (FORMAT JSON) %%'
      LIMIT 10;`)).WillReturnRows(sqlmock.NewRows([]string{"blocking_pid", "blocked_pid", "blocked_query"}))
