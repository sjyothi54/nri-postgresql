...
const (
	...
	// WaitEvents retrieves wait events and their statistics for AWS RDS
	WaitEventsRds = `WITH current_activity AS (
        SELECT
			sa.pid,                                         
        ...
        WHERE
            sa.state != 'idle' AND
            sa.query IS NOT NULL   
        SELECT
            event_type || ':' || COALESCE(event, 'none') AS wait_event_name, -- Concatenated wait event name
            CASE
        ...
        to_char(NOW() AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"') AS collection_timestamp, a.query_text                                                 
