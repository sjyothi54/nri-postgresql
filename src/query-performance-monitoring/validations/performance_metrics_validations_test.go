...
func TestCheckBlockingSessionMetricsFetchEligibilityExtensionNotRequired(t *testing.T) {
	...
	version := uint64(12)
	isExtensionEnabledTest, _ := CheckBlockingSessionMetricsFetchEligibility(conn, version, nil)
...
func TestIndividualQueryMetricsFetchEligibilityUnSupportedVersion(t *testing.T) {
	conn, mock := connection.CreateMockSQL(t)
	version := uint64(11)
	isExtensionEnabledTest, _ := CheckIndividualQueryMetricsFetchEligibility(conn, version, nil)
	assert.Equal(t, isExtensionEnabledTest, false)
	assert.NoError(t, mock.ExpectationsWereMet())
}
...
func TestCheckWaitEventMetricsFetchEligibilityUnsupportedVersion(t *testing.T) {
	conn, mock := connection.CreateMockSQL(t)
	version := uint64(11)
	isExtensionEnabledTest, _ := CheckWaitEventMetricsFetchEligibility(conn, version, nil)
	assert.Equal(t, isExtensionEnabledTest, false)
	assert.NoError(t, mock.ExpectationsWereMet())
}
...
func TestCheckSlowQueryMetricsFetchEligibilityUnSupportedVersion(t *testing.T) {
	conn, mock := connection.CreateMockSQL(t)
	version := uint64(11)
	isExtensionEnabledTest, _ := CheckSlowQueryMetricsFetchEligibility(conn, version, nil)
	assert.Equal(t, isExtensionEnabledTest, false)
	assert.NoError(t, mock.ExpectationsWereMet())
}
